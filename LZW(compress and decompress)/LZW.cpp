    ...
    for(int i=0;i<str1.size();++i)
    {
      ...
      for(pair<string,int>itera:lzw_dictionary)
      {
        ...
        if(itera.first==sum)
        {
        ...
        else
          {
            continue;  /* if not.. it  will loop over lzw_dictionary for searching for it is exist
                          and if not the flag will be Flase */
          }
      }
        ...
        if(flag==1)
        {
          ...
          sum.clear();            // clear sum from any previos value for new loop....
        }
        else    /// sum not exist  >>>
        {
...
void Lzw_decompress(int num)
{
  ...
  /// *** 1st loop to print value of the first int key value(compress_index[0]...
   ...
   for(auto const &x:lzw_dictionary)
        {
            current_int=compress_index[0];                // current_int  equel to first number(key) in compress_index)...
            if(x.first==current_int)                     // to search about it in dictionary...
            {
                ...
                current_str.clear();
                alternat_int=0;                        //  alternat_int sure to be equel to zero to take new value(not very important but for ensure)...
                alternat_int=current_int;              //  alternat_int will take value of current_int...
    ...
    {
        current_int=compress_index[i];        // current_int will take the value of  current key and  alternat_int will be the previous value  of current_int as they alternate value with current_int after each end of loop...


       /// *** 3rd loop to get string value of the key stored in  alternate key and put it in alternat_str...

        for(auto const &x:lzw_dictionary)
        {
            if(x.first==alternat_int)
            {
                alternat_str=x.second;    // alternat_str equal string value of key...
                break;                   //  because if key in begging not continue loop(for time execution)...
       ...
       /// *** 4rd loop to get string value of the key stored in  current key and put it in current_str...
                ...
                /// uncompression ERRORS(SPECIAL CASE OF DECOMPRESS)....
             ...
             /*
              ...
              only case in which this special case can occur is if the substring begins and ends with the same character
               ...
               take the substring you have so far, "ab", and concatenate its first character to itself, "ab"+"a" = "aba",
                instead of following the procedure as normal. Therefore the pseudocode provided above must be altered a bit in order to handle all cases.
            */
       ...
       /// *** Adding new string to dictionary
          ...
          /// the properties of string that add to dictionary must be: the value of alternat_str all (char or string) but current_str must be char only if it string will take only first index [0]...
